...
#define _GLIBCXX_WIDEN(S) _GLIBCXX_WIDEN_(_CharT, S)
    ...
    struct basic_format_string
    {
        ...
        consteval basic_format_string(const _Tp &__s);
    ...
    [[__gnu__::__always_inline__]]
    inline __format::_Dynamic_format_string<char> dynamic_format(string_view __fmt) noexcept
    {
        return __fmt;
    }
    ...
    #ifdef _GLIBCXX_USE_WCHAR_T
    [[__gnu__::__always_inline__]]
    inline __format::_Dynamic_format_string<wchar_t> dynamic_format(wstring_view __fmt) noexcept
    {
        return __fmt;
    }
    #endif
    ...
    {
      public:
        _GLIBCXX_CONSTEXPR_FORMAT_ERROR explicit format_error(const string &__what) : runtime_error(__what) {}

        _GLIBCXX_CONSTEXPR_FORMAT_ERROR explicit format_error(const char *__what) : runtime_error(__what) {}
    };

    /// @cond undocumented
    [[noreturn]]
    inline void __throw_format_error(const char *__what)
    {
...
#undef _GLIBCXX_CONSTEXPR_FORMAT_ERROR
    ...
    namespace __format
    {
        ...
        [[noreturn]]
        inline void __unmatched_left_brace_in_format_string()
        {
            __throw_format_error("format error: unmatched '{' in format string");
        }

        [[noreturn]]
        inline void __unmatched_right_brace_in_format_string()
        {
            __throw_format_error("format error: unmatched '}' in format string");
        }

        [[noreturn]]
        inline void __conflicting_indexing_in_format_string()
        {
            __throw_format_error("format error: conflicting indexing style in format string");
        }

        [[noreturn]]
        inline void __invalid_arg_id_in_format_string()
        {
            __throw_format_error("format error: invalid arg-id in format string");
        }

        [[noreturn]]
        inline void __failed_to_parse_format_spec()
        {
    ...
    class basic_format_parse_context
    {
      public:
        ...
        constexpr void advance_to(const_iterator __it) noexcept
        {
            _M_begin = __it;
        }
    ...
    namespace __format
    {
        ...
        template<typename _Context>
        size_t __int_from_arg(const basic_format_arg<_Context> &__arg);
        ...
        struct _Spec : _SpecBase
        {
            ...
            // pre: __first != __last
            static constexpr iterator _S_parse_width_or_precision(iterator __first,
                                                                  iterator __last,
            ...
            constexpr iterator _M_parse_precision(iterator __first, iterator __last, basic_format_parse_context<_CharT> &__pc)
            {
                ...
                bool __arg_id = false;
                if(__next != __last)
                    __next = _S_parse_width_or_precision(__first, __last, _M_prec, __arg_id, __pc);
                if(__next == __first)
            ...
            template<typename _Context>
            size_t _M_get_width(_Context &__ctx) const
            {
                size_t __width = 0;
            ...
            template<typename _Context>
            size_t _M_get_precision(_Context &__ctx) const
            {
                size_t __prec = -1;
        ...
        inline char *__put_sign(_Int __i, _Sign __sign, char *__dest) noexcept
        {
            if(__i < 0)
                *__dest = '-';
        ...
        inline _Out __write(_Out __out, basic_string_view<_CharT> __str)
        {
        ...
        _Out __write_padded_as_spec(basic_string_view<type_identity_t<_CharT>> __str,
        ...
        template<typename _CharT>
        size_t __truncate(basic_string_view<_CharT> &__s, size_t __prec)
        {
        ...
        struct _Escapes
        {
            ...
            static consteval _Str_view _S_all()
            {
                return _GLIBCXX_WIDEN("\t\\t\n\\n\r\\r\\\\\\\"\\\"'\\'\\u\\x");
            }

            static consteval _Str_view _S_tab()
            {
                return _S_all().substr(0, 3);
            }

            static consteval _Str_view _S_newline()
            {
                return _S_all().substr(3, 3);
            }

            static consteval _Str_view _S_return()
            {
                return _S_all().substr(6, 3);
            }

            static consteval _Str_view _S_bslash()
            {
                return _S_all().substr(9, 3);
            }

            static consteval _Str_view _S_quote()
            {
                return _S_all().substr(12, 3);
            }

            static consteval _Str_view _S_apos()
            {
                return _S_all().substr(15, 3);
            }

            static consteval _Str_view _S_u()
            {
                return _S_all().substr(18, 2);
            }

            static consteval _Str_view _S_x()
            {
        ...
        struct _Separators
        {
            ...
            static consteval _Str_view _S_squares()
            {
                return _S_all().substr(0, 2);
            }

            static consteval _Str_view _S_braces()
            {
                return _S_all().substr(2, 2);
            }

            static consteval _Str_view _S_parens()
            {
                return _S_all().substr(4, 2);
            }

            static consteval _Str_view _S_comma()
            {
                return _S_all().substr(6, 2);
            }

            static consteval _Str_view _S_colon()
            {
        ...
        template<typename _Out, typename _CharT>
        _Out __write_escape_seq(_Out __out, uint_least32_t __val, basic_string_view<_CharT> __prefix)
        {
            using _Str_view = basic_string_view<_CharT>;
        ...
        template<typename _Out, typename _CharT>
        _Out __write_escape_seqs(_Out __out, basic_string_view<_CharT> __units)
        {
        ...
        template<typename _Out, typename _CharT>
        _Out __write_escaped_char(_Out __out, _CharT __c)
        {
            using _UChar = make_unsigned_t<_CharT>;
            using _Esc = _Escapes<_CharT>;
            switch(__c)
        ...
        template<typename _CharT, typename _Out>
        _Out __write_escaped_ascii(_Out __out, basic_string_view<_CharT> __str, _Term_char __term)
        {
            using _Str_view = basic_string_view<_CharT>;
            ...
            while(__first != __last)
            {
                ...
                while(__print != __last && !__format::__should_escape_ascii(*__print, __term))
                    ++__print;
        ...
        template<typename _CharT, typename _Out>
        _Out __write_escaped_unicode_part(_Out __out, basic_string_view<_CharT> &__str, bool &__prev_esc, _Term_char __term)
        {
            using _Str_view = basic_string_view<_CharT>;
            using _Esc = _Escapes<_CharT>;
            ...
            while(__first != __last)
            {
                ...
                {
                    if(*__it <= 0x7f)
                        return __esc_ascii = __format::__should_escape_ascii(*__it.base(), __term);
        ...
        template<typename _CharT, typename _Out>
        _Out __write_escaped_unicode(_Out __out, basic_string_view<_CharT> __str, _Term_char __term)
        {
        ...
        template<typename _CharT, typename _Out>
        _Out __write_escaped(_Out __out, basic_string_view<_CharT> __str, _Term_char __term)
        {
        ...
        struct _Optional_locale
        {
            ...
            _Optional_locale(const _Optional_locale &__l) noexcept : _M_dummy(), _M_hasval(__l._M_hasval)
            {
                if(_M_hasval)
                    std::construct_at(&_M_loc, __l._M_loc);
            }

            _Optional_locale &operator=(const _Optional_locale &__l) noexcept
            {
                if(_M_hasval)
                {
                    if(__l._M_hasval)
        ...
        struct __formatter_str
        {
            ...
            _Out format(basic_string_view<_CharT> __s, basic_format_context<_Out, _CharT> &__fc) const
            {
                if(_M_spec._M_debug)
            ...
            template<typename _Out>
            _Out _M_format_escaped(basic_string_view<_CharT> __s, basic_format_context<_Out, _CharT> &__fc) const
            {
            ...
            _Out _M_format_range(_Rg &&__rg, basic_format_context<_Out, _CharT> &__fc) const
            {
	    ...
	    case _Pres_c:
	      return _M_format_character(_S_to_character(__i), __fc);
                ...
                if(_M_spec._M_alt && __base_prefix.size())
                {
                    ...
                    __builtin_memcpy(__start, __base_prefix.data(), __base_prefix.size());
                }
                ...
                else
                {
                    ...
                    return _M_format_int(basic_string_view<_CharT>(__wbuf, __n), __prefix_len, __fc);
                }
#endif
            }
            ...
            template<typename _Int>
            static _CharT _S_to_character(_Int __i)
            {
            ...
            template<typename _Out>
            typename basic_format_context<_Out, _CharT>::iterator
            _M_format_int(basic_string_view<_CharT> __str, size_t __prefix_len, basic_format_context<_Out, _CharT> &__fc) const
            {
                    ...
                    if(__l.name() != "C")
                    {
                        ...
                        if(!__grp.empty())
                        {
                            ...
                            auto __end = std::__add_grouping(
                                __p + __prefix_len, __np.thousands_sep(), __grp.data(), __grp.size(), __s, __s + __n);
...
#undef _GLIBCXX_FORMAT_F128
        ...
        struct __formatter_fp
        {
            ...
            basic_string<_CharT> _M_localize(basic_string_view<_CharT> __str, char __expc, const locale &__loc) const
            {
                ...
                auto __overwrite = [&](_CharT *__p, size_t)
                {
                    ...
                    auto __end = std::__add_grouping(
                        __p + __off, __np.thousands_sep(), __grp.data(), __grp.size(), __str.data() + __off, __str.data() + __e);
        ...
        struct __formatter_ptr
        {
            ...
            constexpr __formatter_ptr(_Spec<_CharT> __spec) noexcept : _M_spec(__spec)
            {
...
// P2510R3 Formatting pointers
          ...
          private:
            [[__gnu__::__always_inline__]]
            constexpr void _M_set_default(_Pres_type __type)
            {
    ...
    struct formatter<char, wchar_t>
    {
        ...
        constexpr typename basic_format_parse_context<wchar_t>::iterator parse(basic_format_parse_context<wchar_t> &__pc)
        {
    ...
    constexpr bool enable_nonlocking_formatter_optimization<basic_string<char, _Tr, _Alloc>> = true;
#endif
    ...
    struct formatter<basic_string<wchar_t, _Traits, _Alloc>, wchar_t>
    {
        ...
        template<typename _Out>
        typename basic_format_context<_Out, wchar_t>::iterator
        format(const basic_string<wchar_t, _Traits, _Alloc> &__u, basic_format_context<_Out, wchar_t> &__fc) const
        {
            return _M_f.format(__u, __fc);
        }
    ...
    #if __glibcxx_print >= 202403L
    template<typename _Tr, typename _Alloc>
    constexpr bool enable_nonlocking_formatter_optimization<basic_string<wchar_t, _Tr, _Alloc>> = true;
    #endif
    ...
    constexpr bool enable_nonlocking_formatter_optimization<basic_string_view<char, _Tr>> = true;
#endif
    ...
    struct formatter<basic_string_view<wchar_t, _Traits>, wchar_t>
    {
        ...
        template<typename _Out>
        typename basic_format_context<_Out, wchar_t>::iterator
        format(basic_string_view<wchar_t, _Traits> __u, basic_format_context<_Out, wchar_t> &__fc) const
        {
            return _M_f.format(__u, __fc);
        }
    ...
    #if __glibcxx_print >= 202403L
    template<typename _Tr>
    constexpr bool enable_nonlocking_formatter_optimization<basic_string_view<wchar_t, _Tr>> = true;
    #endif
#endif // USE_WCHAR_T
    /// @}
    ...
    namespace __format
    {
        ...
        template<>
        inline constexpr bool __is_formattable_integer<__int128> = true;
        template<>
        inline constexpr bool __is_formattable_integer<unsigned __int128> = true;
#endif
    ...
    struct formatter<_Tp, _CharT>
    {
        ...
        template<typename _Out>
        typename basic_format_context<_Out, _CharT>::iterator format(_Tp __u, basic_format_context<_Out, _CharT> &__fc) const
        {
            return _M_f.format(__u, __fc);
        }
    ...
    constexpr bool enable_nonlocking_formatter_optimization<_Tp> = true;
#endif
    ...
    struct formatter<__format::__bflt16_t, _CharT>
    {
        ...
        template<typename _Out>
        typename basic_format_context<_Out, _CharT>::iterator
        format(__gnu_cxx::__bfloat16_t __u, basic_format_context<_Out, _CharT> &__fc) const
        {
            return _M_f.format((float)__u, __fc);
        }
    ...
    template<>
    struct formatter<char *, wchar_t> : private __formatter_disabled
    {
    };

    template<>
    struct formatter<const char *, wchar_t> : private __formatter_disabled
    {
    };

    template<size_t _Nm>
    struct formatter<char[_Nm], wchar_t> : private __formatter_disabled
    {
    };

    template<class _Traits, class _Allocator>
    struct formatter<basic_string<char, _Traits, _Allocator>, wchar_t> : private __formatter_disabled
    {
    };

    template<class _Traits>
    struct formatter<basic_string_view<char, _Traits>, wchar_t> : private __formatter_disabled
    {
    };
#endif
    ...
    namespace __format
    {
        ...
        class _Sink_iter
        {
          ...
          public:
            ...
            auto _M_reserve(size_t __n) const
            {
                return _M_sink->_M_reserve(__n);
            }

            bool _M_discarding() const
            {
        ...
        class _Sink
        {
          ...
          protected:
            ...
            [[__gnu__::__always_inline__]]
            span<_CharT> _M_used() const noexcept
            {
            ...
            void _M_reset(span<_CharT> __s, size_t __pos = 0) noexcept
            {
                _M_span = __s;
            ...
            virtual bool _M_discarding() const
            {
                return false;
            }
        ...
        class _Seq_sink : public _Buf_sink<typename _Seq::value_type>
        {
          ...
          protected:
            ...
            typename _Sink<_CharT>::_Reservation _M_reserve(size_t __n) override
            {
            ...
            void _M_bump(size_t __n) override
            {
                if constexpr(__is_specialization_of<_Seq, basic_string> || __is_specialization_of<_Seq, vector>)
                {
                    auto __s = this->_M_used();
          ...
          public:
            ...
            _Seq_sink(_Seq &&__s) noexcept(is_nothrow_move_constructible_v<_Seq>) : _M_seq(std::move(__s)) {}
        ...
        class _Iter_sink : public _Buf_sink<_CharT>
        {
          ...
          protected:
            ...
            bool _M_discarding() const override
            {
          ...
          public:
            [[__gnu__::__always_inline__]]
            explicit _Iter_sink(_OutIter __out, iter_difference_t<_OutIter> __max = -1) :
            ...
            format_to_n_result<_OutIter> _M_finish() &&
            {
                if(this->_M_used().size() != 0)
        ...
        class _Ptr_sink : public _Sink<_CharT>
        {
          ...
          protected:
            ...
            typename _Sink<_CharT>::_Reservation _M_reserve(size_t __n) final
            {
          ...
          private:
            ...
            static size_t _S_trim_max(_IterDifference __max)
            {
                if(__max < 0)
                    return _S_no_limit;
            ...
            [[__gnu__::__always_inline__]]
            void _M_rebuf(_CharT *__ptr, size_t __total, size_t __inuse = 0)
            {
          ...
          public:
            explicit _Ptr_sink(_CharT *__ptr, size_t __n = _S_no_limit) noexcept : _Sink<_CharT>(_M_buf), _M_max(__n)
            {
                if(__n == 0)
            ...
            explicit _Ptr_sink(_OutIter __out, iter_difference_t<_OutIter> __n = -1) :
                _Ptr_sink(std::to_address(__out), _S_trim_max(__n))
            {
            }

            template<contiguous_iterator _OutIter>
            format_to_n_result<_OutIter> _M_finish(_OutIter __first) const
            {
                auto __s = this->_M_used();
                if(__s.data() == _M_buf)
                {
        ...
        class _Padding_sink : public _Str_sink<_CharT>
        {
            ...
            [[__gnu__::__always_inline__]]
            bool _M_ignoring() const
            {
                return _M_printwidth >= _M_maxwidth;
            }

            [[__gnu__::__always_inline__]]
            bool _M_buffering() const
            {
          ...
          public:
            ...
            _Out _M_finish(_Align __align, char32_t __fill_char)
            {
        ...
        class _Escaping_sink : public _Buf_sink<_CharT>
        {
            ...
            void _M_write()
            {
                ...
                if constexpr(__unicode::__literal_encoding_is_unicode<_CharT>())
                {
                    ...
                    _M_out = __format::__write_escaped_ascii(std::move(_M_out), __str, _M_term);
          ...
          public:
            [[__gnu__::__always_inline__]]
            explicit _Escaping_sink(_Out __out, _Term_char __term) :
        ...
        struct _Arg_value
        {
            ...
            class handle
            {
                ...
                explicit handle(_Tp &__val) noexcept :
              ...
              public:
                ...
                [[__gnu__::__always_inline__]]
                void format(basic_format_parse_context<_CharT> &__pc, _Context &__fc) const
                {
	...
	{ _S_get<_Tp>() = __val; }
            ...
            [[__gnu__::__always_inline__]]
            static auto &_S_access(_Self &__u, _Value... __value) noexcept
            {
            ...
            template<typename _Tp>
            [[__gnu__::__always_inline__]]
            const auto &_M_get() const noexcept
            {
                return _S_access<_Tp>(*this);
            }
    ...
    class basic_format_arg
    {
      ...
      public:
        ...
        _Res visit(this basic_format_arg __arg, _Visitor &&__vis)
        {
            return __arg._M_visit_user(std::forward<_Visitor>(__vis), __arg._M_type);
        }
#endif
      ...
      private:
        ...
        friend auto make_format_args(_Argz &...) noexcept;

        template<typename _Visitor, typename _Ctx>
        friend decltype(auto) visit_format_arg(_Visitor &&__vis, basic_format_arg<_Ctx>);

        template<typename _Visitor, typename _Ctx>
        friend decltype(auto) __format::__visit_format_arg(_Visitor &&, basic_format_arg<_Ctx>);

        template<typename _Ch, typename _Tp>
        friend consteval __format::_Arg_t __format::__to_arg_t_enum() noexcept;

        template<typename _Visitor>
        decltype(auto) _M_visit(_Visitor &&__vis, __format::_Arg_t __type)
        {
            using namespace __format;
            switch(__type)
            {
        ...
        template<typename _Visitor>
        decltype(auto) _M_visit_user(_Visitor &&__vis, __format::_Arg_t __type)
        {
            return _M_visit(
                [&__vis]<typename _Tp>(_Tp &__val) -> decltype(auto)
                {
    ...
    inline decltype(auto) visit_format_arg(_Visitor &&__vis, basic_format_arg<_Context> __arg)
    {
        return __arg._M_visit_user(std::forward<_Visitor>(__vis), __arg._M_type);
    }
    ...
    {
        template<typename _Visitor, typename _Ctx>
        inline decltype(auto) __visit_format_arg(_Visitor &&__vis, basic_format_arg<_Ctx> __arg)
        {
        ...
        {
            template<typename _Tp>
            size_t operator()(_Tp &__arg) const
            {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        template<typename _Context>
        inline size_t __int_from_arg(const basic_format_arg<_Context> &__arg)
        {
    ...
    class basic_format_args
    {
        ...
        size_t _M_size() const noexcept
        {
            return _M_packed_size ? _M_packed_size : _M_unpacked_size;
        }

        typename __format::_Arg_t _M_type(size_t __i) const noexcept
        {
        ...
        template<typename _Ctx, typename... _Args>
        friend auto make_format_args(_Args &...) noexcept;
    ...
    template<typename _Context, typename... _Args>
    auto make_format_args(_Args &...__fmt_args) noexcept;
    ...
    class __format::_Arg_store
    {
        ...
        template<typename _Tp>
        static _Element_t _S_make_elt(_Tp &__v)
        {
        ...
        [[__gnu__::__always_inline__]] _Arg_store(_Tp &...__a) noexcept : _M_args{_S_make_elt(__a)...}
        {
        }
    };
    ...
    template<typename _Context>
    template<typename... _Args>
    inline basic_format_args<_Context>::basic_format_args(const _Store<_Args...> &__store) noexcept
    {
    ...
    [[nodiscard, __gnu__::__always_inline__]]
    inline auto make_format_args(_Args &...__fmt_args) noexcept
    {
    ...
    template<typename... _Args>
    [[nodiscard, __gnu__::__always_inline__]]
    inline auto make_wformat_args(_Args &...__args) noexcept
    {
    ...
    namespace __format
    {
        ...
        _Out
        __do_vformat_to(_Out, basic_string_view<_CharT>, const basic_format_args<_Context> &, const locale * = nullptr);
    ...
    class basic_format_context
    {
        ...
        basic_format_context(basic_format_args<basic_format_context> __args, _Out __out) :
            _M_args(__args), _M_out(std::move(__out))
        {
        }

        basic_format_context(basic_format_args<basic_format_context> __args, _Out __out, const std::locale &__loc) :
        ...
        friend _Out2
        __format::__do_vformat_to(_Out2, basic_string_view<_CharT2>, const basic_format_args<_Context2> &, const locale *);
      ...
      public:
        ...
        [[nodiscard]]
        basic_format_arg<basic_format_context> arg(size_t __id) const noexcept
        {
    ...
    namespace __format
    {
        ...
        struct _Scanner
        {
            ...
            constexpr explicit _Scanner(basic_string_view<_CharT> __str, size_t __nargs = (size_t)-1) :
                _M_pc(__str, __nargs)
            {
            }
            ...
            constexpr basic_string_view<_CharT> _M_fmt_str() const noexcept
            {
                return {begin(), end()};
            }
        ...
        {
          public:
            _Formatting_scanner(basic_format_context<_Out, _CharT> &__fc, basic_string_view<_CharT> __str) :
        ...
        {
            static_assert((is_default_constructible_v<formatter<_Args, _CharT>> && ...),
...
// most common use cases.
        ...
        __do_vformat_to<wchar_t, 1>(_Sink_iter<wchar_t>, wstring_view, wformat_context &);
    #endif
#endif

        template<typename _Out, typename _CharT, typename _Context>
        inline _Out
        __do_vformat_to(_Out __out, basic_string_view<_CharT> __fmt, const basic_format_args<_Context> &__args, const locale *__loc)
        {
            if constexpr(is_same_v<_Out, _Sink_iter<_CharT>>)
            {
                auto __ctx = __loc == nullptr ? _Context(__args, __out) : _Context(__args, __out, *__loc);
        ...
        __do_vformat_to_n(_Out __out,
                          iter_difference_t<_Out> __n,
                          basic_string_view<_CharT> __fmt,
                          const type_identity_t<basic_format_args<__format_context<_CharT>>> &__args,
...
#pragma GCC diagnostic pop
    ...
    template<typename _Tp>
        requires convertible_to<const _Tp &, basic_string_view<_CharT>>
    consteval basic_format_string<_CharT, _Args...>::basic_format_string(const _Tp &__s) : _M_str(__s)
    {
    ...
    [[__gnu__::__always_inline__]]
    inline _Out vformat_to(_Out __out, string_view __fmt, format_args __args)
    {
    ...
    [[__gnu__::__always_inline__]]
    inline _Out vformat_to(_Out __out, wstring_view __fmt, wformat_args __args)
    {
        return __format::__do_vformat_to(std::move(__out), __fmt, __args);
    }
#endif
    ...
    template<typename _Out>
        requires output_iterator<_Out, const char &>
    [[__gnu__::__always_inline__]]
    inline _Out vformat_to(_Out __out, const locale &__loc, string_view __fmt, format_args __args)
    {
    ...
    template<typename _Out>
        requires output_iterator<_Out, const wchar_t &>
    [[__gnu__::__always_inline__]]
    inline _Out vformat_to(_Out __out, const locale &__loc, wstring_view __fmt, wformat_args __args)
    {
        return __format::__do_vformat_to(std::move(__out), __fmt, __args, &__loc);
    }
#endif
    ...
    [[nodiscard]]
    inline string vformat(string_view __fmt, format_args __args)
    {
...
#ifdef _GLIBCXX_USE_WCHAR_T
    [[nodiscard]]
    inline wstring vformat(wstring_view __fmt, wformat_args __args)
    {
    ...
    template<typename... _Args>
    [[nodiscard]]
    inline string format(format_string<_Args...> __fmt, _Args &&...__args)
    {
    ...
    template<typename... _Args>
    [[nodiscard]]
    inline wstring format(wformat_string<_Args...> __fmt, _Args &&...__args)
    {
    ...
    template<typename... _Args>
    [[nodiscard]]
    inline string format(const locale &__loc, format_string<_Args...> __fmt, _Args &&...__args)
    {
        return std::vformat(__loc, __fmt.get(), std::make_format_args(__args...));
    }

#ifdef _GLIBCXX_USE_WCHAR_T
    template<typename... _Args>
    [[nodiscard]]
    inline wstring format(const locale &__loc, wformat_string<_Args...> __fmt, _Args &&...__args)
    {
        return std::vformat(__loc, __fmt.get(), std::make_wformat_args(__args...));
    }
#endif

    template<typename _Out, typename... _Args>
        requires output_iterator<_Out, const char &>
    inline _Out format_to(_Out __out, format_string<_Args...> __fmt, _Args &&...__args)
    {
        return std::vformat_to(std::move(__out), __fmt.get(), std::make_format_args(__args...));
    }

#ifdef _GLIBCXX_USE_WCHAR_T
    template<typename _Out, typename... _Args>
        requires output_iterator<_Out, const wchar_t &>
    inline _Out format_to(_Out __out, wformat_string<_Args...> __fmt, _Args &&...__args)
    {
        return std::vformat_to(std::move(__out), __fmt.get(), std::make_wformat_args(__args...));
    }
#endif

    template<typename _Out, typename... _Args>
        requires output_iterator<_Out, const char &>
    inline _Out format_to(_Out __out, const locale &__loc, format_string<_Args...> __fmt, _Args &&...__args)
    {
        return std::vformat_to(std::move(__out), __loc, __fmt.get(), std::make_format_args(__args...));
    }

#ifdef _GLIBCXX_USE_WCHAR_T
    template<typename _Out, typename... _Args>
        requires output_iterator<_Out, const wchar_t &>
    inline _Out format_to(_Out __out, const locale &__loc, wformat_string<_Args...> __fmt, _Args &&...__args)
    {
        return std::vformat_to(std::move(__out), __loc, __fmt.get(), std::make_wformat_args(__args...));
    }
#endif
    ...
    template<typename _Out, typename... _Args>
        requires output_iterator<_Out, const char &>
    inline format_to_n_result<_Out>
    format_to_n(_Out __out, iter_difference_t<_Out> __n, format_string<_Args...> __fmt, _Args &&...__args)
    {
        return __format::__do_vformat_to_n(std::move(__out), __n, __fmt.get(), std::make_format_args(__args...));
    }
    ...
    template<typename _Out, typename... _Args>
        requires output_iterator<_Out, const wchar_t &>
    inline format_to_n_result<_Out>
    format_to_n(_Out __out, iter_difference_t<_Out> __n, wformat_string<_Args...> __fmt, _Args &&...__args)
    {
        return __format::__do_vformat_to_n(std::move(__out), __n, __fmt.get(), std::make_wformat_args(__args...));
    }
#endif
    ...
    template<typename _Out, typename... _Args>
        requires output_iterator<_Out, const char &>
    inline format_to_n_result<_Out>
    format_to_n(_Out __out, iter_difference_t<_Out> __n, const locale &__loc, format_string<_Args...> __fmt, _Args &&...__args)
    {
        return __format::__do_vformat_to_n(std::move(__out), __n, __fmt.get(), std::make_format_args(__args...), &__loc);
    }
    ...
    template<typename _Out, typename... _Args>
        requires output_iterator<_Out, const wchar_t &>
    inline format_to_n_result<_Out>
    format_to_n(_Out __out, iter_difference_t<_Out> __n, const locale &__loc, wformat_string<_Args...> __fmt, _Args &&...__args)
    {
        return __format::__do_vformat_to_n(std::move(__out), __n, __fmt.get(), std::make_wformat_args(__args...), &__loc);
    }
#endif
    ...
    template<typename... _Args>
    [[nodiscard]]
    inline size_t formatted_size(format_string<_Args...> __fmt, _Args &&...__args)
    {
        __format::_Counting_sink<char> __buf;
        std::vformat_to(__buf.out(), __fmt.get(), std::make_format_args(__args...));
        return __buf.count();
    }
    ...
    template<typename... _Args>
    [[nodiscard]]
    inline size_t formatted_size(wformat_string<_Args...> __fmt, _Args &&...__args)
    {
        __format::_Counting_sink<wchar_t> __buf;
        std::vformat_to(__buf.out(), __fmt.get(), std::make_wformat_args(__args...));
        return __buf.count();
    }
#endif
    ...
    template<typename... _Args>
    [[nodiscard]]
    inline size_t formatted_size(const locale &__loc, format_string<_Args...> __fmt, _Args &&...__args)
    {
        __format::_Counting_sink<char> __buf;
        std::vformat_to(__buf.out(), __loc, __fmt.get(), std::make_format_args(__args...));
        return __buf.count();
    }
    ...
    template<typename... _Args>
    [[nodiscard]]
    inline size_t formatted_size(const locale &__loc, wformat_string<_Args...> __fmt, _Args &&...__args)
    {
        __format::_Counting_sink<wchar_t> __buf;
        std::vformat_to(__buf.out(), __loc, __fmt.get(), std::make_wformat_args(__args...));
        return __buf.count();
    }
#endif
    ...
    namespace __format
    {
        ...
        struct __indexed_formatter_storage
        {
            ...
            template<typename _Out>
            void _M_format(__maybe_const<_Tp, _CharT> &__elem,
                           basic_format_context<_Out, _CharT> &__fc,
        ...
        class __tuple_formatter
        {
          ...
          protected:
            ...
            {
                return __format::__format_padded(__fc,
                                                 _M_spec,
          ...
          private:
            ...
            struct __formatters_storage : __indexed_formatter_storage<_Ids, _Tps, _CharT>...
            {
                ...
                template<typename _Out>
                void _M_format(__maybe_const<_Tps, _CharT> &...__elems,
                               basic_format_context<_Out, _CharT> &__fc,
            ...
            template<size_t... _Ids>
            static auto _S_create_storage(index_sequence<_Ids...>) -> __formatters_storage<_Ids...>;
    ...
    template<__format::__char _CharT, formattable<_CharT> _Fp, formattable<_CharT> _Sp>
    struct formatter<pair<_Fp, _Sp>, _CharT> :
        __format::__tuple_formatter<_CharT, remove_cvref_t<_Fp>, remove_cvref_t<_Sp>>
    {
      private:
        using __maybe_const_pair =
            __conditional_t<formattable<const _Fp, _CharT> && formattable<const _Sp, _CharT>, const pair<_Fp, _Sp>, pair<_Fp, _Sp>>;
      ...
      public:
        ...
        template<typename _Out>
        typename basic_format_context<_Out, _CharT>::iterator
        format(__maybe_const_pair &__p, basic_format_context<_Out, _CharT> &__fc) const
        {
