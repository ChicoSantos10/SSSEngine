...
namespace SSSEngine
{
    SSSENGINE_FORCE_INLINE constexpr bool PointersOverlap(const void *first, const void *second, Size bytes) noexcept
    {
    ...
    SSSENGINE_FORCE_INLINE void MemoryCopy(const From *SSSENGINE_RESTRICT from, To *SSSENGINE_RESTRICT to, Size bytes) noexcept
    {
    ...
    SSSENGINE_FORCE_INLINE void MemorySet(T *to, i8 value, Size bytes) noexcept
    {
#ifdef SSSENGINE_MSVC
#elif SSSENGINE_CLANG || SSSENGINE_GCC
    ...
    template<typename T>
        requires(IsBitwiseCopyable<T>)
    SSSENGINE_FORCE_INLINE void ZeroMemory(T *to, Size bytes) noexcept
    {
     ...
     */
    template<typename From, typename To>
    SSSENGINE_FORCE_INLINE int MemoryCompare(const From *from, const To *to, Size bytes) noexcept
    {
#ifdef SSSENGINE_MSVC
#elif SSSENGINE_CLANG || SSSENGINE_GCC
     ...
     * @tparam From The first Type to compare
     * @param from First memory address
     * @param to Value to compare
     * @param bytes Amount of bytes to compare
     * @return -1 if first is less than compare value, 0 if the same, 1 if second is greater than first
     */
    template<typename From>
    SSSENGINE_FORCE_INLINE int MemoryCompare(const From *from, const u8 compareValue, Size bytes) noexcept
    {
